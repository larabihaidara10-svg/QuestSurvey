...
class Database {
    ...
    private function initMainDatabase() {
        $this->pdo->exec("
            ...
            CREATE TABLE IF NOT EXISTS questionnaires (
                ...
                collect_student_number INTEGER DEFAULT 0,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    public function queryResponse($sql, $params = []) {
        $stmt = $this->responsesDb->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    
    public function queryResponseOne($sql, $params = []) {
        $stmt = $this->responsesDb->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    
    public function executeResponse($sql, $params = []) {
        $stmt = $this->responsesDb->prepare($sql);
        return $stmt->execute($params);
    }
    ...
    public function generateAccessCodes($questionnaireId, $count) {
        for ($i = 0; $i < $count; $i++) {
            ...
            $this->execute("INSERT INTO access_codes (questionnaire_id, code) VALUES (?, ?)", [$questionnaireId, $code]);
        }
    }
    
    public function validateAccessCode($questionnaireId, $code) {
        return $this->queryOne("SELECT id FROM access_codes WHERE questionnaire_id = ? AND code = ? AND is_used = 0", [$questionnaireId, $code]);
    }
    
    public function markAccessCodeUsed($codeId) {
        $this->execute("UPDATE access_codes SET is_used = 1, used_at = CURRENT_TIMESTAMP WHERE id = ?", [$codeId]);
    }
    
    public function getAccessCodes($questionnaireId) {
        return $this->query("SELECT code, is_used, used_at FROM access_codes WHERE questionnaire_id = ? ORDER BY id", [$questionnaireId]);
    }
}